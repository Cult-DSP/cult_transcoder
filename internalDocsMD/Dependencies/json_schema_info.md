...
> **Deprecated as of v0.5.2.** The old `renderInstructions.json` format is no longer generated by the pipeline. The C++ renderer now reads `scene.lusid.json` directly. The old format is preserved in `src/packageADM/old_schema/createRenderInfo.py` for reference.
