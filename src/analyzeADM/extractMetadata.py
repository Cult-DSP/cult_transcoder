...
# Lives at: src/adm_extract/build/spatialroot_adm_extract
# relative to the project root (two levels up from this file's directory).
# ---------------------------------------------------------------------------
_PROJECT_ROOT = Path(__file__).resolve().parent.parent.parent
_EMBEDDED_EXTRACTOR = _PROJECT_ROOT / "src" / "adm_extract" / "build" / "spatialroot_adm_extract"


def extractMetaData(wavPath, outXmlPath):
    """Extract ADM XML from a BW64/RF64/WAV file and write it to outXmlPath.

    Uses the embedded spatialroot_adm_extract binary (built via init.sh).
    ...
    if not _EMBEDDED_EXTRACTOR.exists():
        raise FileNotFoundError(
            f"spatialroot_adm_extract not found at {_EMBEDDED_EXTRACTOR}\n"
